    ...
    const [eingabee, setEingabee] = useState({
      ...
      useEffect(() => {
        const fetchData2 = async () => {
            setLoading(true);
            try {
                 const response2 = await Service.getPraktikumListe();
      ...
      <tr key={pflicht.id}>
       ...
       <div className="items-center justify-center h-30 w-full my-4">
          <label className="block text-gray-600 text-sm font-normal">
            Wählen Sie ein Praktikum aus.
          </label>
          <input
             value={eingabee.eingabePraktikum}
            type={"text"}
            name="eingabePraktikum"
